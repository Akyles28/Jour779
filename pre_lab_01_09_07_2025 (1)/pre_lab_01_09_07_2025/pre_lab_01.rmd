...
# It returns[1] 4, like in the previous task.
...
#There are 79,366 obs. and nine variables
...
# The "id" column is <chr> and the "seats" is <dbl>
 ...
 **Answer**
# The name of the entry from the first row of the "title" column is Directed Research.
...
# The first line identifies the dataset we're looking at then we group the data by information in the "term" column. Then we set up summaries for a count of the number of classes for each term we're grouping.
...
#The results are now arranged in order of the highest value to the lowest.
...
#Instead of being grouped by term, it's grouped by term and department. Also, it's arranged by term and department. 
...
#In the summarise function, we added the sum fuction to find the total number of seats offered each term. So now the results show us the number of classes and the amout of seats they offer each term. 
...
# The top terms have similar means of seats. These results helped us see the average amount of seats per term, adding more columns than the previous results in the last codeblock. 
...
# This codeblock added the minimum number of seats and the maximum number of seats per department. The data is sorted by the number of seat per department per term. 
...
1.  How many seats are there per instructor? 

2.  Which department has the highest amount of seats? 

3.  Which department has the highest count of classes offered? 
