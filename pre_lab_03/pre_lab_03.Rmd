...
options(scipen=999)
```
###Akira Kyles 9/22/25, Jour 772
## About this notebook

This notebook contains code and explanatory text that you should review and run as you read through two chapters on data cleaning from the course textbook, "Data Journalism with R and the Tidyverse". Answer questions and edit the document as directed.
...
###The first line of the red warning says "one or more parsing issues, call "problems()' on your data frame for details". After running the 'problems(payments)' code, it generated a table of the parsing problems. 
...
###No, it doesn't show any parsing errors when I run the code this time.
...
###They are all labeled as characters.
...
###There are 553 rows of code that do not have five numbers. The first number was a 0, so it was cut from the data. 
...
###The results for the category column aren't organized and some of the results say 'NA'.One entry just day 'L' instead of loan. Every year has loan and grant information except 2009. 
...
###Two loan amounts are under $100 and majority are over $1,000. 
...
###The headers are no longer inconsistent with capitalization and got rid of spacing. 
...
### clean_names funtion standardizes, then the column name was changed, then the capitalization was changed to uppercase for each column. 
...
###There are 58 possible duplicates. 
...
### The new data frame has 19,453 rows. 
...
###Theres a new column called zip5 that has consistent 5-digit zip codes. 
...
### The 21202 zip code has gotten the most amount of money from the Maryland Tourism Board. That zip code is in Baltimore. That zip code doesn't make sense to me as the top recipient. As a former resident of Baltimore, and frequent visitor, there does not appear to be much money focused of all or even the majority of that zip code. 
```{r}
cleaned_md_grants_loans |> 
